...
<h2>
<openmrs:message code="VisitType.title"/>
</h2>
...
<table>
	...
	<tr>
		...
		<td valign="top">
			<spring:bind path="visitType.description">
				<textarea name="description" rows="3" cols="40" data-maxlength="1024">
				${status.value}
				</textarea>
				<c:if test="${status.errorMessage != ''}">
				<span class="error">
	...
	<c:if test="${!(visitType.creator == null)}">
		<tr>
			...
			<td>
			<openmrs:format user="${ visitType.creator }"/>
			</td>
		</tr>
	</c:if>
	<tr>
        <c:if test="${visitType.visitTypeId != null}">
           	...
           	<font color="#D0D0D0">
           	<sub>
           	<openmrs:message code="general.uuid"/>
           	</sub>
           	</font>
           	</td>
...
<c:if test="${not visitType.retired && not empty visitType.visitTypeId}">
	<form method="post">
		<fieldset>
			<h4>
			<openmrs:message code="VisitType.retireVisitType"/>
			</h4>
			...
			<spring:hasBindErrors name="visitType">
				<c:forEach items="${errors.allErrors}" var="error">
					...
					<span class="error">
					<openmrs:message code="${error.defaultMessage}" text="${error.defaultMessage}"/>
					</span>
					</c:if>
				</c:forEach>
			</spring:hasBindErrors>
...
<c:if test="${visitType.retired && not empty visitType.visitTypeId}">
	<form method="post">
		<fieldset>
			<h4>
			<openmrs:message code="VisitType.unretireVisitType"/>
			</h4>
...
<c:if test="${not empty visitType.visitTypeId}">
	<openmrs:hasPrivilege privilege="Manage Visit Types">
		<form id="purge" method="post" onsubmit="return confirmPurge()">
			<fieldset>
				<h4>
				<openmrs:message code="VisitType.purgeVisitType"/>
				</h4>
